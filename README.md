...
- 전체 벤치마크는 **6개 도메인(부가가치세·법인세·소득세·국세기본법·회계·복합 agent) 302문항**으로 구성되며, 각 도메인이 역량 매트릭스(정확·근거·실무·리스크·도구) 목표 셀을 충족합니다(설계 청사진 §6-4).
- [data/sample-questions-v0.1.jsonl](data/sample-questions-v0.1.jsonl): `public_sample` **49문항** (전체 302문항 중 공개분)
- [leaderboard/public/data/public/release.jsonl](leaderboard/public/data/public/release.jsonl): canary 포함 공개 release bundle
- 리더보드 순위는 private holdout 집계 기준입니다. `closed_book` 302문항 재평가 점수와 `authority_rag-v1` 별도 view가 공개 리더보드에 반영되어 있습니다.
- Authority RAG는 공개된 별도 view이지만, benchmark-provided frozen source pack을 쓰는 다른 평가 조건이므로 `closed_book` 공식 순위 평균에는 합산하지 않습니다.
- holdout 문항의 본문, 정답, id는 공개하지 않습니다.

이 저장소는 K-TaxBench의 public distribution입니다. 실행기, 문서, 공개 샘플, 공개 리더보드 payload를 포함하며, 전체 holdout set과 raw model outputs는 별도의 private 운영 환경에서 관리합니다.
...
아래 명령은 모델 API나 CLI 인증 없이 공개 샘플과 테스트 suite가 정상인지 확인합니다.
...
먼저 `config/models.yaml`에 모델을 등록합니다.
...
OpenAI/Google SDK provider를 쓰려면 provider extra와 API key가 필요합니다.

```bash
uv sync --extra dev --extra providers
```

그 다음 공개 샘플 평가를 실행합니다.
...
공식 leaderboard 등재는 self-reported score를 받지 않습니다. 제출자는 model id, provider, mode, prompt/scaffold version, retrieval/tool 설정을 고정하고, maintainer가 동일 실행기와 private holdout으로 재현 채점합니다. 재제출은 기존 행을 덮어쓰지 않고 새 submission row로 추가합니다.
...
이 public distribution은 private holdout이나 local credential을 포함하지 않도록 관리합니다. 공개 전 기본 경계 검사는 다음과 같습니다.
...
- Full benchmark size: **302 questions across 6 domains** (VAT, corporate tax, income tax, basic tax law, accounting, mixed agent), each domain meeting its capability-matrix target cells (accuracy / grounding / practice / risk / tools)
- Public sample data: [data/sample-questions-v0.1.jsonl](data/sample-questions-v0.1.jsonl)
- Public sample rows in this repository: **49** `public_sample` questions (the public subset of the 302-question benchmark)
...
- Leaderboard ranking is based on private holdout aggregates; holdout question text, answers, and ids are not exposed. (Published scores reflect the most recent measurement run; re-evaluation over the expanded question bank is updated in a separate measurement cycle.)

## Quickstart: Validate The Public Sample

These commands do not require model API keys or CLI authentication. They validate the public sample and test suite.
...
First, register your model in `config/models.yaml`.
...
For OpenAI/Google SDK providers, install provider extras and set the required API key.

```bash
uv sync --extra dev --extra providers
```

Then run the public sample evaluation.
...
Official leaderboard entries do not use self-reported scores. Submitters freeze model id, provider, mode, prompt/scaffold version, and retrieval/tool settings. The maintainer then reproduces the run with the same harness and the private holdout. Resubmissions create new submission rows instead of overwriting older results.
