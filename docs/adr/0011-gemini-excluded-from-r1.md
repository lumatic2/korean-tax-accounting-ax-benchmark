...
- **(a) Antigravity CLI (`agy`)** — Gemini 3.x(3.5 Flash·3.1 Pro) 를 가졌으나 **에이전트형 구조**(tool-permission·conversation·`--add-dir`, TTY/GUI 세션 의존)다. non-TTY subprocess(배치 러너가 쓰는 방식)에서 `agy --print` 가 **응답 없이 행(hang)** 하거나 stdin 리다이렉트 시 무출력 종료. claude-cli/codex 같은 "단발 completer + 깨끗한 stdout" 모델이 아니라 스크립트 어댑터로 못 씀. 사용자 인증 TTY 점검에서도 동일(`phases/m4r1-multi-provider/HANDOFF-agy.md`).
...
- ⚠ ADR 0010·ROADMAP R1 의 "≥3 프로바이더" 목표 → 2-벤더로 축소. 3번째 벤더는 reviewer 강력 요구 시 Gemini **API 키 경로**(SDK 어댑터 잔존)로 후속.
