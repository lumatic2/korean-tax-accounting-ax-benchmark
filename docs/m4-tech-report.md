...
> 규칙: 모든 수치·결론은 레포 내 finding/ADR/DOMAIN 또는 공식 출처를 인용한다. 수치는 finding 발표값을 그대로 인용하며 본문에서 재산출하지 않는다.
...
K-TaxBench는 이 다섯 실패 모드를 분해해 측정하는 평가 인프라다. 한국 세무·회계 실무를 6개 도메인 × 7개 과제유형으로 분해한 문항(현행 302문항, 본 리포트의 측정 결과는 초기 v0.1 101문항 셋 기준)을, **정확성·근거성·실무성·리스크 인식·도구 사용성**의 다차원 루브릭으로 채점한다. 채점은 코드 채점(객관식·계산·근거 locator의 결정론적 채점)과 LLM-judge(7차원 루브릭)를 결합하되, 평가받는 모델이 자기 답을 채점하지 못하도록 judge를 후보군 밖 모델로 고정하고 후보·judge 실행 환경을 격리한다.
...
본 리포트 측정 시점의 v0.1 문항셋은 101문항이었다(현행 벤치마크는 6개 도메인 302문항으로 확장됨 — [README.md](../README.md) 참조; `data/sample-questions-v0.1.jsonl`).
...
평가받는 모델이 자기 답을 채점하면 점수에 천장(self-preference)이 생긴다. 이를 막기 위해 **judge는 항상 후보군 밖 모델**로 둔다(예: opus 후보 → sonnet judge; sonnet 후보 → opus judge).
...
코드 채점(계산 룰 프록시)은 self-eval 천장을 넘는 외부 검증 사례를 만들어냈다. 양도소득 문항에서 오케스트레이터가 자산구분 오류(소득세법 제104①11호 → 제1호)를 교정했고, 기부금 문항에서 final 값과 explanation의 모순을 포착했다. 모델 self-judgment로는 잡히지 않는, 결정론적 산식 대조만이 잡는 유형의 오류다.
...
- **검수 단계.** 공개 34문항에 대한 본인 검수 라운드를 완료했다(2026-06-12; law-mcp/kifrs 원문 대조로 정확성·근거성·적시성·내부정합성·루브릭 5축 검증). 다음 단계는 외부 세무·회계 전문가 검수 프로토콜 도입이며, 전문가 검수를 통과한 문항만 `version:1.0`으로 승격한다.
...
- **멀티프로바이더 + 단일 패밀리 한계.** 1차 범위는 Claude 계열 3모델 변별에 집중했고 judge도 Claude 계열이다. 따라서 (a) 변별 spread는 *동일 패밀리 내부* 관측이며 프로바이더 간 일반화는 미검증이고, (b) non-self judge 안전장치도 "same-family non-self" 수준이라 패밀리 단위 선호 편향(same-family preference)을 완전히 배제하지 못한다. GPT·Gemini 교차 변별 + judge-swap robustness가 후속 핵심 과제다([adr/0002](adr/0002-claude-cli-first.md)).
...
| §7–8 한계·재현 | [adr/0002](adr/0002-claude-cli-first.md)·[0009](adr/0009-leaderboard-submission-policy.md) |
